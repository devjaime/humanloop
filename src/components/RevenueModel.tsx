...
const icons = [DollarSign, CheckCircle, Users, TrendingUp];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl font-black mb-4">
            <span className="text-text-primary">{t.revenue.title1} </span>
            <span className="bg-gradient-to-r from-success to-electric bg-clip-text text-transparent">
              {t.revenue.titleHighlight}
            </span>
          </h2>
          <p className="text-text-secondary max-w-2xl mx-auto text-lg">{t.revenue.subtitle}</p>
        </div>

        <div className="grid md:grid-cols-4 gap-6 mb-16">
          {t.revenue.stats.map((stat, index) => {
            const Icon = icons[index] ?? DollarSign;
            return (
              <div key={stat.label} className="bg-surface-light border border-border rounded-2xl p-6 text-center">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-16">
          {t.revenue.steps.map((item) => (
            <div key={item.step} className="relative bg-surface-light border border-border rounded-2xl p-8">
